...
export default function Booking() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Basic Validation
    if (!fullName || !phoneNumber || !service || !date || !preferredTime) {
      setError('Please fill in all required fields.');
      return;
    }

    // Phone Number Validation: simple check for digits, spaces, plus, and hyphens (10-15 chars)
    ...
    if (additionalNotes.length > 500) {
      setError('Additional notes cannot exceed 500 characters.');
      return;
    }

    // Secure Data Handling (Simulation)
    // Normally we would send this to a backend. Since this is frontend only,
    // we just reset and show success.
              ...
              <form className="space-y-6" onSubmit={handleSubmit} noValidate>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-white border border-brand-200 px-4 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all text-brand-900 appearance-none cursor-pointer"
                  >
